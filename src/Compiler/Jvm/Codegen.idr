...
mutual
  ...
  handleRet "void" op = op ++ lineSeparator ++ mkWorld (schConstructor (NS ["Builtin"] (UN "MkUnit")) 0 [])
...
compileToSS c tm outfile
    = do ds <- getDirectives Chez
         ...
         depTree <- buildFunctionTreeMain defs
         coreLift $ IO.putStrLn $ displayTree (show . jvmName) depTree
         coreLift $ printLn $ jvmName <$> dfs depTree
         compdefs <- traverse (getJvmBytecode assembler chezExtPrim chezString defs) ns
...
||| Codegen wrapper for JVM bytecode implementation.
export
codegenJvm : Codegen
codegenJvm = MkCG compileExpr executeExpr
