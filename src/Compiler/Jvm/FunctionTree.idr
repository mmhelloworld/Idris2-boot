...
parameters (defs: SortedMap Name NamedDef)
    mutual
        ...
        buildFunctionTreeConAlt acc visited ((MkNConAlt n tag args sc) :: alts) =
        ...
        buildFunctionTreeExp acc visited (NmCon fc name tag args) = buildFunctionTreeCApp acc visited args
        ...
        buildFunctionTreeExp acc visited (NmForce fc t) = buildFunctionTreeExp acc visited t
        buildFunctionTreeExp acc visited (NmDelay fc t) = buildFunctionTreeExp acc visited t
        ...
        buildFunctionTree visitedSoFar name =
            ...
            in case SortedMap.lookup name defs of
               Nothing => (visited, Node name [])
               Just d => let (visited, children) = buildFunctionTreeDef visited name d
                    in (visited, Node name children)

buildFunctionTreeMain : SortedMap Name NamedDef -> Tree Name
buildFunctionTreeMain defs = snd $ buildFunctionTree defs SortedSet.empty idrisMainFunctionName
