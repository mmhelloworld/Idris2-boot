...
%hide Prelude.Interactive.putStrLn

mockRunAsm : AsmState -> Asm a -> Core (a, AsmState)
...
mockRunAsm state (ClassCodeStart version access className sig parent intf anns) = assemble state $ do
...
mockRunAsm state (ClassCodeEnd out) =
    assemble state $ putStrLn $ "classCodeEnd " ++ out
mockRunAsm state (CreateClass opts) =
    assemble state $ putStrLn $ "createClass " ++ show opts
mockRunAsm state (CreateField accs className fieldName desc sig fieldInitialValue) = assemble state $ do
  ...
  putStrLn $ unwords [
    ...
    desc,
    (fromMaybe "" sig),
    !(Objects.toString (maybeToNullable (toJFieldInitialValue <$> fieldInitialValue)))]
...
mockRunAsm state (InvokeDynamic mname desc handle bsmArgs) = assemble state $ do
  ...
  putStrLn $ unwords [
    ...
    mname,
    desc,
    !(Objects.toString $ the Object $ believe_me jhandle),
    !(Objects.toString $ the Object $ believe_me jbsmArgs)]
...
mockRunAsm state (Lconst c) = assemble state $
    putStrLn $ "lconst " ++ show c

mockRunAsm state (Ldc (TypeConst ty)) =
    assemble state $ putStrLn $ "ldcType " ++ ty
mockRunAsm state (Ldc constant) = assemble state $ do
    putStrLn ("ldc " ++ !(Objects.toString (constantToObject constant)))
...
mockRunAsm state (LookupSwitch defaultLabel labels cases) = assemble state $ do
  ...
  putStrLn $ unwords [
    "lookupSwitch",
    defaultLabel,
    !(Objects.toString (the Object $ believe_me jlabels)),
    !(Objects.toString (the Object $ believe_me jcases))]
...
mockRunAsm state (Throw fc message) = pure (crash $ show fc ++ ": " ++ message, state)
...
mockRunAsm state action = pure (crash "Unsupported acton", state)